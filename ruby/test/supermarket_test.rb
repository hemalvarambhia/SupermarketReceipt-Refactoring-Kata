...
class SupermarketTest < Minitest::Test
  ...
  def setup
    ...
    @toothbrush = Kata::Product.each(name: 'toothbrush')
    @catalog.add_product(@toothbrush, 0.99)
    @rice = Kata::Product.each(name: 'rice')
    @catalog.add_product(@rice, 2.99)
    @apples = Kata::Product.kilo_of(name: 'apples')
    @catalog.add_product(@apples, 1.99)
    @cherry_tomatoes = Kata::Product.each(name: 'cherry tomato box')
  ...
  def test_two_normal_items
    @the_cart.add_item_quantity(@toothbrush, 1)
    @the_cart.add_item_quantity(@rice, 1.0)
    receipt = @teller.checks_out_articles_from(@the_cart)
    verify Kata::ReceiptPrinter.new(40).print_receipt(receipt)
  end
